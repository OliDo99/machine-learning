...
CAR_IMAGE = pygame.transform.scale(pygame.image.load("Images/Car2.png"),(100,50))
...
class AbstractCar:
    ...
    def draw(self,SCREEN):
        blit_rotate_center(SCREEN,self.img,(self.x-50,self.y-25),self.ang)
    ...
    def radar(self,radar_ang):
        ...
        while not SCREEN.get_at((x, y)) == pygame.Color(BORDER_COLOR) and length < 100:
            length += 1
            x = int(x+ math.cos(math.radians(self.ang+radar_ang)) * length)
            y = int(y - math.sin(math.radians(self.ang+radar_ang)) * length)
        pygame.draw.line(SCREEN, (255, 255, 255), (self.x,self.y), (x, y), 1)
        pygame.draw.circle(SCREEN, (0, 0, 255, 0), (x, y), 3)

        dist = int(math.sqrt(math.pow(x - x, 2)+ math.pow(y - y, 2)))
